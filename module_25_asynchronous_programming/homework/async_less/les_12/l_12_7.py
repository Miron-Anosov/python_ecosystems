...
async def place_order(request: Request) -> Response:
    ...
    await order_queue.put(Order(user_type, randrange(5)))  # Преобразовать запрос в заказ
    return Response(body='Order placed!')


async def create_order_queue(app: Application):
    print('Создание заказа, очереди и задачи.')
...
"""
Этот код очень похож на первоначальный API для взаимодействия с медленной подсистемой управления заказами, но теперь в 
нем используется очередь с приоритетами и создается класс Order, представляющий заказ. Мы ожидаем, что во входящем 
запросе будет флаг "power_user", равный True для приоритетных пользователей и False для всех остальных.
"""
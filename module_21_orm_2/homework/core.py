...
def get_books_by_authors_that_is_not_read(id_student: int) -> json:
    ...
    # как понимать другие произведения автора, если по заданию мы выборку делаем по студенту?
    # Может тогда идет речь не об авторе, а обо всех авторах?
    #  Не понятно каким образом мне решать какого автора возвращать.
    """
    # TODO Имеется ввиду, составить список книг авторов, книги которых студент ранее брал, за исключением прочитанных книг
    books = [
